/**
 * OpenStreetMap Reverse Geocoding Utility (Nominatim)
 */
...
export async function fetchAddressFromCoordinates(
  ...
  try {
    ...
    const response = await fetch(url, {
      headers: {
        "Accept-Language": "vi",
        // Setting a descriptive User-Agent header as requested by OSM Nominatim usage policy
        "User-Agent": "QuickBite-App/1.0",
      },
    });
